...
- `build.js`: Script de pré-processamento. Ele varre recursivamente a pasta `dados/`, lê todos os arquivos `.xls` e `.csv`, extrai e otimiza apenas os campos necessários, remove duplicatas parciais e gera o arquivo `data.json`.
- `dados/`: Pasta contendo os arquivos organizados por fonte:
  - `scraper-SUAPCNPQ/`: Arquivos Excel extraídos do Lattes (`[SIGLA-CAMPUS]_[ANO-INICIAL]_[ANO-FINAL].xls`)
  - `scraper-DGP/`: Arquivo CSV do Diretório de Grupos de Pesquisa (`coletor_dgp_ifba.csv`)
- `data.json`: Arquivo consolidado e otimizado servido para a interface pública.

## Como Atualizar os Dados

1. Coloque ou substitua as planilhas do Lattes (`[SIGLA-CAMPUS]_[ANO-INICIAL]_[ANO-FINAL].xls`) na pasta `dados/scraper-SUAPCNPQ/` e o arquivo DGP (`coletor_dgp_ifba.csv`) na pasta `dados/scraper-DGP/`. O sistema detecta o período automaticamente pelo nome do arquivo.
